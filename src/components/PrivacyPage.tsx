...
interface PrivacyPageProps {
  initialData?: { data: PrivacyPolicyData; meta: Record<string, unknown> };
}


const PrivacyPage: React.FC<PrivacyPageProps> = ({ initialData }) => {
        ...
        >
          <motion.div variants={fadeInUp} className="flex flex-row items-center justify-between mb-12 border-b border-gray-100 pb-8">
            ...
            <div className="text-xs font-bold uppercase tracking-[0.3em] text-ag-green-950">
              Data Privacy & Security — {new Date(cmsData?.last_updated ?? '').getFullYear()}
            </div>
          </motion.div>
              ...
              <div className="lg:col-span-5 pb-4">
                ...
                <motion.div variants={fadeInUp} className="flex gap-8">
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm font-bold text-ag-green-950">{cmsData?.version_label}</p>
                  </div>
                  <div className="flex flex-col gap-1">
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest leading-none">Scope</p>
                    <p className="text-sm font-bold text-ag-green-950">Global Site & Portal</p>
                  </div>
                </motion.div>
              </div>
          ...
          <aside className="lg:col-span-3 hidden lg:block sticky top-32 pb-20 self-start">
            ...
            {cmsData?.support_card_title && (
              <div className="mt-20 p-10 rounded-[2.5rem] bg-gray-50 border border-gray-100">
                <p className="text-xs font-bold text-ag-green-950 uppercase tracking-[0.2em] mb-4">
                  {cmsData.support_card_title}
                </p>
